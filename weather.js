...
function updateWeather(regionCode) {
  const url = `${baseUrl}${regionCode}.json`;
...
function updateWeather(regionCode) {
    ...
    const existingTextNode = locationDiv.querySelector(".locationText");
    if (existingTextNode) {
    ...
    fetch(url)
      ...
      .then(weather => {
        ...
        locationText.textContent = `${weather[1].publishingOffice}: ${weather[1].timeSeries[0].areas[0].area.name} `;
